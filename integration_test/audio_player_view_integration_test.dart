...
void main() {
  ...
  group('AudioPlayerView undo/redo tests', () {
    testWidgets('Single undo/redo of forward 1 minute position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
    ...
    testWidgets('Single undo/redo of forward 10 seconds position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
      const String toSelectAudioTitle =
    ...
    testWidgets('Single undo/redo of backward 1 minute position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
      const String toSelectAudioTitle =
    ...
    testWidgets('Single undo/redo of backward 10 seconds position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
      const String toSelectAudioTitle =
    ...
    testWidgets('Single undo/redo of forward slider position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
      const String toSelectAudioTitle =
      ...
      // skip to start of the audio
      await tester
          .tap(find.byKey(const Key('audioPlayerViewSkipToStartButton')));
      await tester.pumpAndSettle();

      // change the current audio's play position using slider

      // Find the slider
      Slider sliderWidget = tester.widget<Slider>(find.byType(Slider));

      // Verify the initial position is 0 seconds
      expect(sliderWidget.value, 0);
      expect(sliderWidget.min, 0);
      expect(sliderWidget.max, 1232);

      // Drag the slider to the right
      // Assuming the total duration of the audio is known
      const int totalDurationInSeconds = 1232;
      const int seekToSeconds = 120;

      // Calculate the offset for the drag gesture
      final double sliderWidth = tester.getSize(find.byType(Slider)).width;
      final double pixelsPerSecond = sliderWidth / totalDurationInSeconds;
      final Offset dragOffset = Offset(pixelsPerSecond * seekToSeconds / 6.175, 0);

      // Perform the drag gesture
      await tester.drag(find.byType(Slider), dragOffset);
      await tester.pumpAndSettle();

      // check the current audio's changed position
      expect(tester.widget<Slider>(find.byType(Slider)).value, 741);
    ...
    testWidgets(
        'Single undo/redo new version of forward slider position change',
        (WidgetTester tester) async {
      const String audioPlayerSelectedPlaylistTitle =
          'S8 audio'; // Youtube playlist
      const String toSelectAudioTitle =
...
// Verify the initial position is 0 (assuming the slider starts at the beginning)
      expect(sliderWidget.value, 0);
      ...
      final double sliderWidth = tester.getSize(find.byType(Slider)).width;
      final double pixelsPerSecond = sliderWidth / totalDurationInSeconds;
      final Offset dragOffset = Offset(pixelsPerSecond * seekToSeconds, 0);
      ...
      // Check the current audio's changed position
      double expectedSliderValue = seekToSeconds / totalDurationInSeconds;
      expect(tester.widget<Slider>(find.byType(Slider)).value,
          closeTo(expectedSliderValue, 0.01));
